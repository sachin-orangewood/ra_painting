...
for a in range(5):
    def func2():
        ...
        oy = [45,43,41,39,37,35,33,31,29,27,25,23]
        ...
        for y in oy:
            ...
            oldtheta2=-math.degrees(math.acos((ox*ox+y*y-(l1*l1)-(l2*l2))/ (2*l1*l2)))  
            oldtheta1=math.degrees(math.atan(y/ox) - math.atan((l2*math.sin(oldtheta2*math.pi/180))/(l1 + l2*math.cos(oldtheta2*math.pi/180))))
            oldtheta3=math.degrees(math.acos(oz/(l1*math.cos(oldtheta1*math.pi/180) + l2*math.cos((oldtheta2 + oldtheta1)*math.pi/180))))

            print(" oldtheta1:" + str(oldtheta1)+" oldtheta2:"+ str(oldtheta2)+ " oldtheta3:"+ str(oldtheta3))
            ppr=1600  # Pulse Per Revolution

            nx = horiz_dist ##x+2 ##{45,50,55,60,65}
            ny = y-2 ##height
            nz = 0
            #ny = (0.04 * (nx - x)) - ny
    ...
    def func3():
        ...
        oy = [21,23,25,27,29,31,33,35,37,39,41,43]
        for y in oy:
            ##ox[5] = {40 45 50 55 60}
